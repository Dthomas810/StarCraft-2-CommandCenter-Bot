...
void ProductionManager::manageBuildOrderQueue()
{
    ...
    if (m_queue.isEmpty())
    {
		inGoal = true; // so supply part of deadlock works correctly 
		std::cout << "about to run boGoal from prod. manager" << std::endl;
		UnitPairVector goal = m_bot.Strategy().StrategyManager::getBuildOrderGoal();

		for (auto & unitpair : goal) {  // Questionable naming 
		  //first is the unit/building, second is the number/you want  
			for (int i = 0; i <= unitpair.second; i++) {				
				m_queue.queueAsHighestPriority(MetaType(unitpair.first, m_bot), false);
			}
		}
...
{
	//std::cout << "GetMaxSupply() returns " + m_bot.GetMaxSupply() << std::endl;
    ...
    if (!hasRequired)
    {
		...
		m_queue.queueAsHighestPriority(MetaType(m_bot.Data(currentItem.type).requiredUnits[0], m_bot), false);
        fixBuildOrderDeadlock();
        return;
    }
	...
	// build as many depots as their are cc's and keep them building as long as you are foodForMoreSupply within the cap
	//This many depots
	int terranSupplyToBuild = m_bot.UnitInfo().getUnitTypeCount(Players::Self, UnitType(sc2::UNIT_TYPEID::TERRAN_COMMANDCENTER, m_bot), true); 

	//This much supply away from current cap 
	int foodForMoreSupply = terranSupplyToBuild * 8;

	
	//                                                   39                       ex 30
	//limit's to one depot being built at a time, starts when you're the amount of supply of the next you're building away from the cap
		...
		//{
		//	std::cout << "GetMaxSupply() returns " + std::to_string(m_bot.GetMaxSupply()) << std::endl;
		//	
		//	m_queue.queueAsHighestPriority(MetaType(supplyProvider, m_bot), true);
		//}


	//std::cout << "in goal = " + std::to_string(inGoal) << std::endl;
	/*if (inGoal && foodForMoreSupply >= (m_bot.GetMaxSupply() - m_bot.GetCurrentSupply()) && !m_buildingManager.isBeingBuilt(supplyProvider))
	{
		std::cout << "Adding Supply, GetMaxSupply() returns " + std::to_string(m_bot.GetMaxSupply()) << std::endl;

		m_queue.queueAsHighestPriority(MetaType(supplyProvider, m_bot), false);
	}*/

	if (inGoal &&( m_bot.Data(currentItem.type).supplyCost + 8 ) > (m_bot.GetMaxSupply() - m_bot.GetCurrentSupply()) && !m_buildingManager.isBeingBuilt(supplyProvider))
	{
		m_queue.queueAsHighestPriority(MetaType(supplyProvider, m_bot), true);
	}
}
